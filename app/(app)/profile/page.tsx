...
export default function ProfilePage() {
  ...
  async function upgradeAccount() {
    ...
    if (upgradePassword.length < 6) { setUpgradeError("Lösenordet måste vara minst 6 tecken."); return; }
    setUpgrading(true);
    setUpgradeError("");
    const { error } = await supabase.auth.updateUser({ email: upgradeEmail, password: upgradePassword });
    if (error) { setUpgradeError(error.message); setUpgrading(false); }
      ...
      <main className="px-4 py-6 max-w-lg mx-auto space-y-4">
        ...
        {isAnonymous && (
          <div className="bg-yellow-50 border border-yellow-200 rounded-2xl px-4 py-4 space-y-3">
            <div>
              <p className="text-sm font-semibold text-yellow-800">Du är inloggad som gäst</p>
              <p className="text-xs text-yellow-700 mt-0.5">Dina rundor sparas, men om du byter enhet förlorar du åtkomsten. Skapa ett konto för att säkra din data.</p>
            </div>
            {upgradeDone ? (
              <div className="bg-green-50 border border-green-200 rounded-xl px-3 py-2">
                <p className="text-sm text-green-800 font-medium">Bekräftelsemail skickat!</p>
                <p className="text-xs text-green-700 mt-0.5">Klicka på länken i mejlet för att aktivera ditt konto.</p>
              </div>
            ) : (
              <div className="space-y-2">
                <input type="email" placeholder="E-post" value={upgradeEmail}
                ...
                <input type="password" placeholder="Välj lösenord (minst 6 tecken)" value={upgradePassword}
                ...
                <button onClick={upgradeAccount} disabled={upgrading || !upgradeEmail || !upgradePassword}
                  className="w-full bg-yellow-500 text-white rounded-xl py-2 text-sm font-semibold disabled:opacity-50">
                  {upgrading ? "Sparar..." : "Skapa konto med dessa uppgifter"}
                </button>
              </div>
            )}
          </div>
